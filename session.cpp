...
void Session::destroySession()
{
    ...
    if (_pReferenceCounter != NULL && _pReferenceCounter->Release() == 0)
    {
        ...
        if (_privateKeyPassword != NULL)
        {
            ...
            Utils::secureCleanMemory((BYTE*)_privateKeyPassword, strlen(_privateKeyFilename) + 1);
        }
    }
    else
    {
...
void Session::deriveMacKey(BYTE* macKeyBuffer)
{
    ...
    if (!CryptoWrapper::deriveKey_HKDF_SHA256(NULL, NULL, _sharedDhSecretBuffer, DH_KEY_SIZE_BYTES, (const BYTE*)keyDerivationContext, MAX_CONTEXT_SIZE, macKeyBuffer, 64))
    {
        printf("error in deriving MAC key\n");
        exit(0);
    }
}
...
void Session::deriveSessionKey()
{
    ...
    if (!CryptoWrapper::deriveKey_HKDF_SHA256(NULL, NULL, _sharedDhSecretBuffer, DH_KEY_SIZE_BYTES, (const BYTE*)keyDerivationContext, MAX_CONTEXT_SIZE, _sessionKey, SYMMETRIC_KEY_SIZE_BYTES))
    {
        printf("error in deriving MAC key\n");
        exit(0);
    }
...
ByteSmartPtr Session::prepareSigmaMessage(unsigned int messageType)
{
    ...
    CryptoWrapper::signMessageRsa3072Pss(conacatenatedPublicKeysSmartPtr, conacatenatedPublicKeysSmartPtr.size(), privateKeyContext, signature, SIGNATURE_SIZE_BYTES);
    ...
    if (messageType == 2)
        CryptoWrapper::getDhSharedSecret(_dhContext, _remoteDhPublicKeyBuffer, DH_KEY_SIZE_BYTES, _sharedDhSecretBuffer, DH_KEY_SIZE_BYTES);

    // derive the mac
    BYTE macKeyBuffer[64];
    deriveMacKey(macKeyBuffer);
    CryptoWrapper::hmac_SHA256(macKeyBuffer, 64, certBufferSmartPtr, certBufferSmartPtr.size(), calculatedMac, HMAC_SIZE_BYTES);
...
bool Session::verifySigmaMessage(unsigned int messageType, const BYTE* pPayload, size_t payloadSize)
{
    ...
    if (!CryptoWrapper::checkCertificate(rootCaCertBuffer, rootCaCertBuffer.size(), parts[1].part, parts[1].partSize, _expectedRemoteIdentityString))
    {
        printf("error in checking certificate\n");
        return false;
    }
    ...
    if (!CryptoWrapper::getPublicKeyFromCertificate(parts[1].part, parts[1].partSize, &remotepublicKeyContext))
    {
        printf("error in getting public key from certificate\n");
        return false;
    }

    // finally, verify the signature
    bool result = false;
    if (!CryptoWrapper::verifyMessageRsa3072Pss(conacatenatedPublicKeysSmartPtr, conacatenatedPublicKeysSmartPtr.size(), remotepublicKeyContext, parts[2].part, parts[2].partSize, &result))
    {
    ...
    if (messageType == 2)
    {
        ...
        {
            printf("error in getting share secret\n");
            return false;
        }
    }
    ...
    BYTE calculatedMac[HMAC_SIZE_BYTES];
    deriveMacKey(macKeyBuffer);
    CryptoWrapper::hmac_SHA256(macKeyBuffer, 64, parts[1].part, parts[1].partSize, calculatedMac, HMAC_SIZE_BYTES);
...
ByteSmartPtr Session::prepareEncryptedMessage(unsigned int messageType, const BYTE* message, size_t messageSize)
{
    ...
    if (!CryptoWrapper::encryptAES_GCM256(_sessionKey, SYMMETRIC_KEY_SIZE_BYTES, message, messageSize, (const BYTE *)(&messageSize), sizeof(messageSize), ciphertext, MESSAGE_BUFFER_SIZE_BYTES, &encryptedMessageSize))
    {
...
bool Session::decryptMessage(MessageHeader* header, BYTE* buffer, size_t* pPlaintextSize)
{
    ...
    if (!CryptoWrapper::decryptAES_GCM256(_sessionKey, SYMMETRIC_KEY_SIZE_BYTES, buffer, ciphertextSize, (const BYTE*)(&plaintextSize), sizeof(plaintextSize), buffer, MESSAGE_BUFFER_SIZE_BYTES, pPlaintextSize))
    {
